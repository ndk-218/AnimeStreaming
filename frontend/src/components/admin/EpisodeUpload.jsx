...
function EpisodeUpload({ uploadData, setUploadData, onNext, onBack, setError, setSuccess }) {
  ...
  const uploadEpisode = async () => {
    ...
    if (duplicateEpisode) {
      setError(`Episode ${episodeData.episodeNumber} already exists in this season`)
      return
    }
    ...
    try {
      ...
      if (response.data.success) {
        setSuccess(`Episode "${episodeData.title}" uploaded successfully!`)
        setUploadData(prev => ({ 
          ...prev, 
  ...
  return (
    <div>
      ...
      {loadingEpisodes ? (
        ...
        <div className="mb-6">
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Existing Episodes ({existingEpisodes.length})</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {existingEpisodes.slice(0, 6).map((episode) => (
              <div key={episode._id} className="bg-gray-50 border border-gray-200 rounded-lg p-3">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-gray-900">Ep {episode.episodeNumber}</span>
                ...
                <p className="text-sm text-gray-700 truncate">{episode.title}</p>
                {episode.duration && (
                  <p className="text-xs text-gray-500 mt-1">{formatDuration(episode.duration)}</p>
                )}
              </div>
            ))}
            {existingEpisodes.length > 6 && (
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex items-center justify-center">
                <span className="text-sm text-gray-500">+{existingEpisodes.length - 6} more episodes</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="space-y-6">
        {/* Episode Metadata */}
        <div>
          <h3 className="text-lg font-semibold text-gray-800 mb-4">Episode Information</h3>
